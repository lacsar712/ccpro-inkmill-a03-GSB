...
class ViscosityAlarmEvent(Base):
    ...
    rule_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("viscosity_alarm_rules.id", ondelete="CASCADE"),
        nullable=False,
    )
    sample_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("viscosity_samples.id", ondelete="CASCADE"), nullable=False
    )
    ...
    rule: Mapped["ViscosityAlarmRule"] = relationship(
        "ViscosityAlarmRule", back_populates="alarm_events"
    )
    sample: Mapped["ViscositySample"] = relationship(
        "ViscositySample", back_populates="alarm_events"
    )
